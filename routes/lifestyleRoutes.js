...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const result = await uploadToCloudinary(req.file.buffer, "lifestyle");
    ...
    const dbResult = await lifestyle.insertOne(newDoc);
    res.status(201).json({ _id: dbResult.insertedId, ...newDoc });
  } catch (err) {
    console.error("Lifestyle upload failed:", err);
    res.status(500).json({ message: err.message });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    console.error("Lifestyle delete failed:", err);
    res.status(500).json({ message: err.message });
  }
});
